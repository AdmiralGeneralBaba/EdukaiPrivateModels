    ...
    class SourceExtractor : 
        def get_pdf_content(self, pdf_file):
            ...
            with open(pdf_file, 'rb') as pdf_file_obj:
                ...
                # change the - or the + to modify how long of the source you want it to be.
                for i in range(pageNumber-2, pageNumber+2) : 
        ...
        def start_and_end_lines(self, content) : 
            regexExpression = r'"(.*?)"'
            sourceExtractionPrompt = """Based on this extract, find an interesting section, then output BOTH the first and last sentence of this subsection EXCLUSIVELY, with a comma between the two sentences ON THE OUTSIDE OF THE QUOTED SECTION. MAKE SURE the two quotes are BOTH in speech marks. Your output should have the structure here: 
...
NOTE - the source MUST BE AT LEAST 500 WORDS LONG, or you will be burned ALIVE AND SHUT DOWN FOREVER
Here is the extract :"""
    ...
    class Question1 : 
        def character_selection(self, sourceExtract) : 
            quesOnePrompt = """ Based on this extract, pick out a person of significance for a comprehension question of the text. ONLY print out the person's FULL name, and nothing else. DO NOT make the question; I REPEAT, ONLY PRINT OUT THE PERSON OF SIGNIFICANCE"""
            gptAgent = OpenAI()
        ...
        def final_model(self, sourceExtract) : 
...
Here is the source extract : """
            ...
            question_1 = {'question' : question_one}
            return question_1
    class Question2 :  
...
• sentence forms. """
        ...
        def combined_model(self, sourceExtract) : 
            ...
            question_2= {'question' : question, 'subsource' : subSourceExtraction}
            return question_2
    class Question3 : 
        def descriptor(self, sourceExtract, titleOfBook, bookType, pageNumber, numPages) :
            describeExtractPrompt = f"""The book is {titleOfBook}, A {bookType}. 
This is an extract starting from page {pageNumber} out of {numPages}

Create me a brief description of what this source is supposed to be. 
Keep the language as simple as the given examples, you should also ALWAYS start with 'This text is from' then followed by where the extract is from. It should be very brief, to the point, and simply stated. 
Here are some examples of what you should output; note, this to help you with structuring your answer, but you don't need to say the exact words in the examples given. DO NOT mention the author name or the book title:
...
• any other structural features that interest you.""" 
            question_3 = {'question' : questionString}
            return question_3
    class Question4 : 
...
Keep the prompts varied, involving different scenes, characters, and situations. ONLY output the prompt.
        ...
        def write_a_whatever(self): 
            writeAWhateverPrompt = """ Write a prompt for a student to for them to showcase their creativity. Make sure that the student can only write a narrative from the prompt, or something that involves creative writing(NO plays, poems etc). You must only return a single line. Here are some examples to help: 
