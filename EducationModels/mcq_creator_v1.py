...
# CREATE THREE FACT FAKE ANSWERS PER FACT. }
    ...
    def extract_real_answers(self, text):
        pattern = re.compile(r'(\d+)\.\s*\{(.*?)\}')
        return re.findall(pattern, text)

    def extract_false_answers(self, text):
        pattern = re.compile(r'(\d+)\.\s*((?:\{\s*.*?\s*\}\s*)+)')
        matches = re.findall(pattern, text)
        return [(match[0], re.findall(r'\{\s*(.*?)\s*\}', match[1])) for match in matches]

    def create_mcq_list(self, questions, real_answers, false_answers):
        mcq_list = []
        ...
        return mcq_list
    def extraction_and_creation_mcq_list(self, questions, real_answers, false_answers) : 
        extracted_questions = self.extract_questions(questions)
        extracted_answers = self.extract_real_answers(real_answers)
        extracted_false_answers = self.extract_false_answers(false_answers)
    ...
    def mcq_creator_v1(self, real_answers, gpt_type) : 
        questions = self.mcq_question_creator(answers, gpt_type)
        false_answers = self.mcq_false_answers_creator(questions, answers, gpt_type)
...
generated_mcq_list = test.mcq_creator_v1(answers, 1)
